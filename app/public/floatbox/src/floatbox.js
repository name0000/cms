...
( function ( TRUE, FALSE, NULL, UNDEFINED ) {
	var
		...
		scriptUrl = parseUrl( pageOptions.scriptPath || select( 'script', 0, -1 ).src ),
		fbPath = scriptUrl.base,
		rexSpace = /\s+/g,
		floatboxClass = 'floatbox',  // default, may be updated in init
		siteOptions,  // set in init
...
///  begin api functions
	...
	function typeOf ( thing, compare ) {
		...
		try {
			...
			else if ( ( win = getOwnerWindow( thing ) ) ) {
				rtn = thing == win.document ? 'document' : 'node';
			}
		}
		catch ( _ ) { }
	...
	function isSmallScreen () {
		...
		var
			...
			height = screen.width == width ? screen.height : document.documentElement.clientHeight;
	...
	function $ ( id, searchDoc ) {
		// Extends getElementById to also look in the parent and in child iframes.
		var
			context = searchDoc || self,
			frames = [ context ],
			rtn = id,  // reflect back passed in dom elements
			i;

		if ( id && typeof id == 'string' ) {  // typeof is quick and handles x-domain entities

			rtn = NULL;
			id = patch( id, '#', '' );  // accept 'id' or '#id'

			if ( !searchDoc ) {  // add immediate family to the search
				context = self.parent;
				if ( context != self ) {
					frames.push( context );
				}
				frames = frames.concat( select( 'iframe' ) );
			}

			for ( i = 0; !rtn && i < frames.length; i++ ) {
				context = getOwnerWindow(  // filters out x-domain
					frames[ i ],
					'document'
				);
				rtn = context && context.getElementById( id );
			}
		}
		return rtn;
	}  // $
	...
	function select ( selector, $baseEl, nth ) {
		...
		$baseEl = $( $baseEl );
		var
			doc = getOwnerWindow( $baseEl, 'document' ) || document,
			rtn = [],
			scope,
			baseId,
			tempId;
	...
	function require ( source, callback, refetch ) {
		...
		if ( isArray( source ) ) {
			for ( i = 0; i < source.length; i++ ) {
				require( source[ i ], NULL, refetch );
			}
			require( NULL, callback );  // after all requested scripts are done
		}

		else if ( ( $script = $( 'fb' + source ) ) ) {
	...
	function activate ( $baseEl, ownerBox ) {
		...
		if ( ( baseSettings = settings.baseSettings ) ) {  // init has run
			...
			if ( hasClass( $baseEl, floatboxClass ) ) {
				containers = [ $baseEl ];
			}
			else if ( baseSettings.activateMedia !== FALSE ) {
				if ( baseSettings.autoGallery !== FALSE ) {
					containerSettings = parseOptions( $baseEl );
					containerSettings.group = containerSettings.group || 'autoGallery';
					attr( $baseEl, 'data-fb-options', makeOptionString( containerSettings ) );
				}
				containers = [ $baseEl ];
			}
...
///  begin shared-on-data functions
	...
	function trigger ( func, param ) {
		...
		if ( func ) {
			if ( typeof func == 'string' ) {
				...
				rtn = func.apply( self, isArray( param ) ? param : [ param ] );
			}
			else if ( isArray( func ) ) {
				rtn = trigger( func.shift(), func );  // [ function, arg1, arg2, ... ]
			}
		}
	...
	function parseUrl ( url, addParams ) {
		...
		var
			...
			rtn = {
				...
				base: parts.slice( 0, -1 ).join( '/' ) + '/',  // full folder path (aka base href)
	...
	function getOwnerWindow ( $node, prop ) {
		...
		try {
			...
			$node = $node.defaultView[ prop || 'self' ];
			return $node;
		}
		catch ( _ ) { }
	}  // getOwnerWindow
	...
	function patch () {
		// Chain string.replace via an array.
		var
			args = [].slice.call( arguments ),
			rtn = args.shift(),
			i;
...
///  begin internal functions
	...
	function init () {
		...
		pageOptions = self.fbOptions || {};  // refetch in case it follows floatbox.js in the doc

		// capture various categories of option settings
		extend( settings, {
			baseSettings: extend(
		...
		floatboxClass = settings.baseSettings.floatboxClass || floatboxClass;
		activate();  // get the click handler in place

		// get (if not already got) and initalize core
		require(
			!data.coreInit && patch( scriptUrl.fullUrl,
				scriptUrl.base + scriptUrl.fileName,
				scriptUrl.base + 'core'
			),
			function() {
				data.coreInit();  // resolve .coreInit at callback runtime
			}
		);
	}  // init
...
///  execute
	...
	// vars shared across all connected frames
		items: items,
		tapDetails: {},  // tapHandler keeps its record of what's going on here
		viewport: {},  // maintained by viewportHandler
		clientOffset: {},  // ditto
		visualOffset: {},  // ditto
		firstEvents: [],  // to be cleaned up when the last or only box is closed
